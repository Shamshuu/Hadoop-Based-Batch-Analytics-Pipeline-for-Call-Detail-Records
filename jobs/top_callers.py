...
def main():
    ...
    output_path = f"/output/top_callers_by_spend/{args.run_id}"
    ...
    try:
        ...
        # Perform aggregation
        top_callers = df.groupBy("caller_id") \
            .agg(spark_sum("charge_amount").alias("total_spend")) \
            .orderBy(col("total_spend").desc()) \
            .limit(100)

        # Write to output (coalesce(1) to make it a single file)
        top_callers.coalesce(1).write.mode("overwrite").csv(output_path, header=False)

        # Output record count
        output_count = top_callers.count()
